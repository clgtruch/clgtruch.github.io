...
var boutonFin = document.getElementById("fin"),
    ...
    jeuEnCours = true,
    listeCalculs = [
        new CalculRep("","Données :<br>(dvv3vv)$/$$/$(dvv4vv)<br>(dvv3vv)$/$$/$(dvv5vv)<br>Figure :<br>i$1",/^si\s+deux\s+droites\s+sont\s+parall[eè]les\s+[àa]\s+une\s+m[eê]me\s+troisi[èe]me\s+droite[,\s]+alors\s+elles\s+sont\s+parall[èe]les\s+entre[-\s]+elles\.?$/i),
        ...
        new CalculRep("","Données :<br>(AB)⊥(AC)<br>(AB)⊥(BE)<br>Figure :<br>i$3",/^si\s+deux\s+droites\s+sont\s+perpendiculaires\s+[àa]\s+une\s+m[êe]me\s+troisi[eè]me\s+droite[,\s]+alors\s+elles\s+sont\s+parall[èe]les\s+entre[-\s]+elles\.?$/i)
    ];
