...
function isNumber(str) {
    if (!(str.valueOf() instanceof RegExp)){
...
function CalculRep(calc, compl = "", result = undefined, type = "auto") {
    ...
    if (type == "auto") {
        if (typeof result[0] == "undefined") {
            this.type = "nb";
            this.autoNb = true;
            this.calc = this.calc.replace(/ /g,"").replace(/de/g,"*").replace(/!([\S\s]+?)\/\/([\S\s]+?)!/g,"(($1)/($2))");
        } else {
            this.autoNb = false;
    ...
    this.display = (calc + (typeof compl != "undefined" ? compl : "")).replace(/\*/g, "×").
        replace(/\//g, "÷").replace(/\.{1,1}/g, ",").replace(/\$\,\$/g, ".").replace(/\$\÷\$/g, "/").replace(/,{2,}/g, function(str){
            return str.replace(/,/g,".");
        }).
        replace(/i\$([0-9]+)/g,"<img src='quest$1.jpg' alt='quest$1img'>").replace(/vv([0-9]+)vv/g,"<span class='indice'>$1</span>").
        replace(/!([\S\s]+?)÷÷([\S\s]+?)!/g,"<div class=\"fraction\"><div class=\"numerateur\">$1</div><div class=\"denominateur\">\
        $2</div></div>");
    ...
    for (var i = 0; i < c; i++) {
        if (this.type != "heure") {
            this.elements.inputs.push(document.createElement(INPUTBALISE));
    ...
    if (DISPLAYID){
        ...
        this.elements.tr.appendChild(this.elements.tdNum);
    }
    this.elements.tdLabel = document.createElement("td");
    this.elements.tdInputs = document.createElement('td');
    this.elements.tdInputs.className = "tdinput";
    this.elements.tr.appendChild(this.elements.tdLabel).parentNode.appendChild(this.elements.tdInputs);
        ...
        for (var i = 0; i < this.elements.inputs.length; i++) {
            if (this.type == "nb") {
                if (this.autoNb) {// SI le résultat a été calculé automatiquement
                    corrects.push(Math.round(eval(this.calc) * 100000) / 100000 == parseFloat(this.elements.inputs[i].value.replace(/,/g,".").replace(/ /g,"")));
                } else {
                ...
                corrects.push(this.result[i].test(this.elements.inputs[i].value.trim()));
            }
        }
...
/////////////////////////////////////////////////////////////////////////////////////
    ...
    //fin de transform radios
    /////////////////////////////////////////////////////////////////////////////////////////

    this.addLabel = function(nbInp, label){
        //Permet de rajouter un label devant un input
        if (nbInp > 0){
            this.elements.tdInputs.insertBefore(document.createElement("br"),this.elements.inputs[nbInp]);
        }
        this.elements.tdInputs.insertBefore(document.createTextNode(label),this.elements.inputs[nbInp]);
    }
